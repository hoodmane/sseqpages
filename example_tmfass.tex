\documentclass{article}
\usepackage[landscape,margin=0cm,top=2cm]{geometry}
\usepackage{sseqpages}
\makeatletter
\begin{document}

%\pgfset{every differential/.show}
\begin{sseqdata}[differentials={->,blue}, yscale=0.8, xscale=0.5, x label step=2]{tmfass}
\cyclestyles{draw,circle,fill,inner sep=0.3ex}
            {draw,rectangle,fill,inner sep=0.3ex}
\degree{-1}{#1}
\xrange{0}{50}
\yrange{0}{21}

\def\sseq@classplacementtransform{\pgftransformrotate{60}}
% normal - 8.5 seconds
%\def\structline(#1)(#2){\draw (#1)--(#2);} % 5.5 seconds
%\def\structline(#1)(#2){} % 4 sec

% 360 + 80 + 36 = 476
% 3 seconds/476  -- a 0.006 second performance cost to each \structline versus \draw, 0.003 second cost to each \draw versus not drawing it
%\place(0,20)
%\place(0,21)
%\structline(0,21-1)(0,21)
\gdef\tower{% 25
    \place(0,0)
    \foreach \y in {1,...,22}{
        \place(0,\y)
        \structline(0,\y-1)(0,\y)
    }
}


%
%\structline(24,11,1)(24,12,1)
%
\gdef\towergroup{%9 + 25 = 36
    \tower
    \place(1,1)
    \place(3,1)
    \place(2,2)
    \place(3,2)
    \place(3,3)
    \place(6,2)
    \structline(3,1,-1)(3,2,-1)
    \structline(3,2,-1)(3,3,-1)
    \structline(0,0,-1)(1,1,-1)
    \structline(1,1,-1)(2,2,-1)
    \structline(2,2,-1)(3,3,-1)
    \structline(0,0,-1)(3,1,-1)
    \structline(3,1,-1)(6,2,-1)
    \structline(0,1,-1)(3,2,-1)
    \structline(0,2,-1)(3,3,-1)
}


\gdef\towergroupa{%25*2+9+21 = 80
    \towergroup
    \place(0,-1)
    \place(1,0)
    \structline(0,-1)(1,0)
    \begin{scope}[xshift=4,yshift=-1]
    \tower
    \end{scope}
%
    \place(6,0)
    \place(6,1)
    \place(7,-1)
    \place(7,0)
    \place(7,1)
    \place(9,0)
    \place(9,1)
    \place(9,2)
    \place(10,0)
    \place(10,1)
    \place(12,0)
    \place(12,1)
    \place(12,2)
    \place(13,1)
    \structline(6,0,-1)(6,1,-1)
    \structline(6,1,-1)(6,2,-1)
    \structline(7,-1,-1)(7,0,-1)
    \structline(7,0,-1)(7,1,-1)
    \structline(9,0,-1)(9,1,-1)
    \structline(9,1,-1)(9,2,-1)
    \structline(10,0,-1)(10,1,-1)
    \structline(12,0,-1)(12,1,-1)
    \structline(12,1,-1)(12,2,-1)

    \structline(4,-1,-1)(7,0,-1)
    \structline(4,0,-1)(7,1,-1)
    \structline(6,0,-1)(9,1,-1)
    \structline(6,1,-1)(9,2,-1)
    \structline(6,0,-1)(7,1,-1)
    \structline(7,-1,-1)(10,0,-1)
    \structline(7,0,-1)(10,1,-1)
    \structline(9,0,-1)(12,1,-1)
    \structline(9,1,-1)(12,2,-1)
    \structline(9,0,-1)(10,1,-1)
    \structline(10,0,-1)(13,1,-1)
    \structline(12,0,-1)(13,1,-1)

    \d2(4,-1)
    \d2(4,0)
    \d2(4,1)
    \d2(7,-1)
    \d2(7,0)
}

\gdef\towergroupb{ %80 + 25 + 15 = 120
    \towergroupa
    \begin{scope}[xshift=8,yshift=-2]
    \tower
    \end{scope}
    \place(10,-1)
    \place(13,-1)
    \place(13,0)
    \place(15,0)
    \place(16,-1)\place(16,-1)
    \place(16,0)
    \place(16,1)
    \place(17,0)
    \place(18,0)
    \place(18,1)
    \structline(10,-1,-1)(10,0,-1)
    \structline(13,0,-1)(13,1,-1)
    \structline(13,-1,-1)(13,0,-1)
    \structline(13,0,-1)(13,1,-1)
    \structline(16,0,-1)(16,1,-1)
    \structline(16,-1,-2)(16,0,-1)
    \structline(16,-1,-1)(16,0,-1)

    \structline(10,-1,-1)(13,0,-1)
    \structline(13,-1,-1)(16,0,-1)
    \structline(13,0,-1)(16,1,-1)
    \structline(15,0,-1)(18,1,-1)
    \structline(15,0,-1)(16,1,-1)
    \structline(16,-1,-1)(17,0,-1)
    \structline(17,0,-1)(18,1,-1)
    \structline(18,1,-1)(18,0,-1)
    \d2(10,0,-1,-1)
}


%\tower
%\begin{scope}[yshift=3]
%\structline(0,1,2)(0,0)
%\place(0,1)
%\place(0,1)
%\place(0,0)
%\end{scope}
%\place(1,0)
%\d3(1,0)

%\begin{scope}[xshift=0,yshift=1]
%\structline(0,0)(1,1)
%\end{scope}



\towergroup % 36

\begin{scope}[xshift=8,yshift=4]
\towergroupa % 80
\end{scope}

\foreach \n in {2,3,4}{
    \begin{scope}[use context,xshift=8*\n,yshift=4*\n]
    \towergroupb % 120
    \end{scope}
}

\d2(18,4)

\d3(17,4)
\d3(18,5)
%\d3(24,6)
\d3(25,8,,-1)
\d3(26,9)

%\d3(33,12)
\d3(34,13)
%\d3(30,6)
\d4(31,8)
\yrange{0}{20}
\end{sseqdata}\vskip-\baselineskip
\printpage[name=tmfass,page=2]
\newpage
\printpage[name=tmfass,page=3]
\newpage
\printpage[name=tmfass,page=4]
\end{document}
