%
%
% TODO: on background layer (what is this good for??)
% \structline, \structlineoptions, \classoptions page keys, \classoptions gen key
%
...
\begin{command}{\replaceclass\oarg{options}(\meta{x},\meta{y}\opt{,\meta{n}})}
...
\begin{command}{\classoptions[\meta{options}](\meta{x},\meta{y}\opt{,\meta{n}})}
...
% TODO: WHY DOES THIS EXAMPLE WORK???
\begin{codeexample}[]
...
\node[name path=myellipse, draw, ellipse, inner sep=4pt, scale=1, rotate fit=26.5,
      fit=(start) (end)] {};
...
\begin{itemize}
\item The title, axes, axes ticks, and axes labels are printed (the appropriate steps are skipped when the |no title|, |no axes|, |no axes ticks|, or |no axes labels| keys are used or if no title or axes labels are provided).
...
has the same result as |\structline(1,0)(2,2)|, except that you can't later use |\structlineoptions| on it. (Note that |\isalive| is currently broken. Eventually I'll fix it. If fixing it would make your life better, let me know and I will do it sooner.)
\end{commandlist}
\end{keylist}
...
\end{sseqpage}
\end{codeexample}
We want each differential to go from the "\detokenize{h_{21}}" vee to the "id" vee, independent of which classes are in the same position of the two vees. The easy way to accomplish this is by giving tags to each of the two vees.
\end{key}
...
\begin{key}{alias=\meta{node name}}
...
\begin{key}{run off={\meta{start arrow}}{\meta{end arrow}}, run off struct lines={\meta{start arrow}}{\meta{end arrow}},run off differentials={\meta{start arrow}}{\meta{end arrow}}}
...
\begin{key}{class pattern=\meta{class pattern name}}
...
\begin{keylist}{differentials, differential style, change differentials}
...
This style applies to labels on classes, differentials, and structlines. All the more specific styles take precedence over it.
\end{keylist}

\begin{keylist}{class labels,class label style,inner class labels,inner class label style,outer class labels,outer class label style}
...
Suppress the axis. Also suppresses axes labels. You can draw your own axes using tikz inside a |scope| environment with the |background| key.
\end{keylist}

\begin{keylist}{no x axis ticks, no y axis ticks, no x axis ticks, draw x axis ticks, draw y axis ticks, draw axes ticks}
Suppress axes ticks (the numbers next to the axes). Only matters if axes are drawn. You can make your own labels using tikz inside a |scope| environment with the |background| key. For instance, you might want to label the axes as 0, $n$, $2n$, \ldots You can achieve this as follows:
\begin{codeexample}[width=8cm]
\begin{sseqpage}[no x axis ticks, x range={0}{5}]
\begin{scope}[background]
    \node at (0,\ymin - 1) {0};
    \node at (1,\ymin - 1) {n};
\foreach \n in {2,..., \xmax}{
