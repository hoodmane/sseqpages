...
\begin{keylist}{page constraint=\meta{predicate},page constraint and=\meta{predicate}, page constraint or=\meta{predicate}}
...
\draw (0,0)--(\x,\y); % this line causes the problem
\end{sseqdata}
\printpage[name=error example] % Gives error: "Undefined control sequence \x"
\end{codeexample}
...
\begin{sseqdata}[name=error example]
\class(0,0)
\addtocontext{\def\x{1}\def\y{2}} % Tell sseqpages about the definitions
\class(\x,\y)
\draw (0,0)--(\x,\y);
\end{sseqdata}
